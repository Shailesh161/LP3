...
public class JobSequencing {
    public static void printJobSequence(Job[] jobs) {
        ...
        //sorting jobs based on profit.(Greedy appraoach)
        Arrays.sort(jobs, new Comparator<Job>() 
        {
            public int compare(Job j1, Job j2) //. We compare the profit of two jobs (j1 and j2) using Integer.compare(j2.profit, j1.profit)
            {
        ...
        char[] result = new char[maxDeadline]; //array stores the job IDs assigned to each time slot.
        boolean[] slot = new boolean[maxDeadline]; //array tracks whether a specific time slot is filled
        Arrays.fill(result, 'X'); //rray with 'X', representing empty slots
        Arrays.fill(slot, false); // array with false, indicating all slots are initially unfilled.
        ...
        //Yes, exactly! If you have three time slots, only the three jobs with the highest profits (among those that can be scheduled) 
        //will be selected and assigned to those slots. This is because:
        //After sorting the jobs by profit in descending order, you are prioritizing the most profitable jobs.
